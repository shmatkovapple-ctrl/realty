	...
	if err == nil {
		...
		for rows.Next() {
			...
			if rows.Scan(&url) == nil {
				l.MediaURLs = append(l.MediaURLs, url)
			}
		}
		if rows.Err() != nil {
			return nil, fmt.Errorf("чтение медиа: %w", rows.Err())
		}
	}
...
func (r *listingRepository) Update(ctx context.Context, l *entity.Listing) error {
	...
	if len(l.MediaURLs) > 0 {
		_, err = tx.Exec(ctx, `DELETE FROM listing_media WHERE listing_id = $1`, l.ID)
		if err != nil {
			return fmt.Errorf("удаление старых медиа: %w", err)
		}

		for i, url := range l.MediaURLs {
			_, err = tx.Exec(ctx, `
				INSERT INTO listing_media (id, listing_id, url, type, sort_order, created_at)
...
func (r *listingRepository) SaveMedia(ctx context.Context, listingID uuid.UUID, urls []string) error {
	tx, err := r.db.Begin(ctx)
	if err != nil {
		return fmt.Errorf("начало транзакции: %w", err)
	}
	...
	_, err = tx.Exec(ctx, `DELETE FROM listing_media WHERE listing_id = $1`, listingID)
	if err != nil {
		return fmt.Errorf("удаление старых медиа: %w", err)
	}
